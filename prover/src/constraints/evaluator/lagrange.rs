...
/// Contains a specific strategy for evaluating the LogUp-GKR related boundary and transition
/// constraints where the divisors' evaluation is batched. These include the constraints for
/// the Lagrange kernel as well as those for the s-column.
pub struct LogUpGkrConstraintsBatchEvaluator<'a, E: FieldElement, A: Air<BaseField = E::BaseField>>
{
    air: &'a A,
...
impl<'a, E, A> LogUpGkrConstraintsBatchEvaluator<'a, E, A>
where
    E: FieldElement,
    A: Air<BaseField = E::BaseField>,
{
    /// Constructs a new [`LogUpGkrConstraintsBatchEvaluator`].
    pub fn new(
        air: &'a A,
        gkr_data: GkrData<E>,
