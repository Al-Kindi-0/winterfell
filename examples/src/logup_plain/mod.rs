...
pub fn get_example(
    ...
    match hash_fn {
        HashFunction::Blake3_192 => {
            Ok(Box::new(LogUp::<Blake3_192>::new(trace_length, num_witness_columns, options)))
        },
        HashFunction::Blake3_256 => {
            Ok(Box::new(LogUp::<Blake3_256>::new(trace_length, num_witness_columns, options)))
        },
        HashFunction::Sha3_256 => {
            Ok(Box::new(LogUp::<Sha3_256>::new(trace_length, num_witness_columns, options)))
        },
        HashFunction::Rp64_256 => {
            Ok(Box::new(LogUp::<Rp64_256>::new(trace_length, num_witness_columns, options)))
        },
        HashFunction::RpJive64_256 => {
            Ok(Box::new(LogUp::<RpJive64_256>::new(trace_length, num_witness_columns, options)))
        },
    }
}

#[derive(Clone, Debug)]
struct LogUp<H: ElementHasher<BaseField = BaseElement>> {
    trace_len: usize,
    num_witness_columns: usize,
...
impl<H: ElementHasher<BaseField = BaseElement>> LogUp<H> {
...
impl<H> Example for LogUp<H>
where
    H: ElementHasher<BaseField = BaseElement> + Sync + Send,
{
    fn prove(&self) -> Proof {
        // create a prover
        let prover = LogUpProver::<H>::new(self.options.clone());
    ...
    fn verify(&self, proof: Proof) -> Result<(), VerifierError> {
        ...
        winterfell::verify::<LogUpAir, H, DefaultRandomCoin<H>, MerkleTree<H>>(
            proof,
            (),
            &acceptable_options,
        )
