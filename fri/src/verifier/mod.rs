...
use crate::{folding::fold_positions, utils::{map_positions_to_indexes, map_position_to_index}, FriOptions, VerifierError};
    ...
    H: ElementHasher<BaseField = B>,
{
    ...
    fn verify_generic<const N: usize>(
        ...
        println!("self.options.num_fri_layers(self.domain_size) {:?}",self.options.num_fri_layers(self.domain_size));
        channel.unbatch::<N>(&positions, domain_size, N, self.layer_commitments.clone());
        for depth in 0..self.options.num_fri_layers(self.domain_size) {
            ...
            eprintln!("Position indexes is {:?}",position_indexes);
            ...
            println!("evaluations at 12780 is {:?}", evaluations[10]);
    ...
    fn verify_generic_query<const N: usize>(
        ...
        let queries = channel.unbatch::<N>(&positions, self.domain_size, self.options.folding_factor(), self.layer_commitments.clone());
        ...
        for (index, position) in positions.iter().enumerate() {
            //We ask the channel for the sequence of (query_value_per_layer, merkle_proof) at each postion.
            ...
            for depth in 0..self.options.num_fri_layers(self.domain_size) {
                ...
                println!("The depth {:?}", depth);

                let folded_pos = cur_pos % target_domain_size;
                println!("folded position {:?}",folded_pos);
                ...
                let xs: [E; N] = folding_roots
                    ...
                    .unwrap();
                let row_poly =
                    polynom::interpolate(&xs, query_values,true);
